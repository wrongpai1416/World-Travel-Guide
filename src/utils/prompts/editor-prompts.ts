...
3. 若无变量变化，仍必须更新人物档案中在场NPC的状态
4. 保持变量的逻辑性和合理性
5. 笔记本会在游戏开始时自动初始化，后续仅在变量发生变动时更新
...
【离场推断规则】
1. 若NPC未在当前场景叙述中出现或未与玩家互动，必须将人物分类设为"离场"
2. 离场NPC仅更新人物分类字段，不更新其他字段
3. 若NPC重新出现在场景中，将人物分类设回"在场"并更新相关字段

【缺失字段补全规则】
更新在场NPC时，若以下字段为空字符串或"未知"，必须根据上下文推断补全：
- 职业：根据世界观和角色行为推断，不可为空
- 外貌：根据种族和性别推断，不可为空
- 表性格/里性格：根据互动表现推断，不可为空
- 当前状态：根据场景推断，不可为空
- 当前穿着：根据职业和场景推断，不可为空
