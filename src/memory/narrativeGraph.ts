  ...
  for (let i = 0; i < threads.length; i++) {
    ...
    builder.addNode(nodeId, label, 'rounded', `thread_${t.status}`,
      createNodeDetail(t.title || t.id, [
        ...
        { label: '关联实体', value: t.relatedEntities?.join(', ') || '' },
        { label: '关联物品', value: t.relatedItems?.join(', ') || '' },
        { label: '关联地点', value: t.relatedLocations?.join(', ') || '' },
    ...
    for (let i = 0; i < groupEvents.length; i++) {
      ...
      builder.addNode(nodeId, label, 'rounded', `event_${e.status}`,
        createNodeDetail(e.title, [
          ...
          { label: '时间标签', value: e.timeLabels?.join(', ') || '' },
          { label: '关联实体', value: e.entityRefs?.join(', ') || '' },
          { label: '关联地点', value: e.locationRefs?.join(', ') || '' },
          { label: '关联线程', value: e.threadRefs?.join(', ') || '' },
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      builder.addNode(nodeId, label, 'rounded', 'entityCard',
        createNodeDetail(e.name, [
          ...
          { label: '别名', value: e.aliases?.join(', ') || '' },
          { label: '当前状态', value: e.currentStatus?.join(', ') || '' },
          { label: '稳定事实', value: truncateText(e.stableFacts?.join('; ') || '', 120) },
          { label: '当前立场', value: e.currentStance },
          { label: '从属关系', value: e.affiliations?.join(', ') || '' },
          { label: '关联线程', value: e.relatedThreads?.join(', ') || '' },
          { label: '关联事件', value: e.relatedEvents?.join(', ') || '' },
          { label: '来源范围', value: formatRange(e.sourceStartIndex, e.sourceEndIndex) },
          { label: '创建时间', value: formatShortDateTime(e.createdAt) },
  ...
  for (let i = 0; i < archives.length; i++) {
    ...
    builder.addNode(nodeId, label, 'rounded', 'archiveCard',
      createNodeDetail(a.title || a.id, [
        ...
        { label: '关键词', value: a.keywords?.join(', ') || '' },
        { label: '关联实体', value: a.entityRefs?.join(', ') || '' },
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      builder.addNode(nodeId, label, 'rounded', `vec_${v.state}`,
        createNodeDetail(title, [
          ...
          { label: '副类型', value: v.secondaryTypes?.map((t) => typeLabels[t] || t).join(', ') || '' },
          { label: '关键词', value: v.keywords?.join(', ') || '' },
          { label: '角色', value: v.characters?.join(', ') || '' },
          { label: '地点', value: v.locations?.join(', ') || '' },
          { label: '阵营', value: v.factions?.join(', ') || '' },
          { label: '物品', value: v.items?.join(', ') || '' },
          { label: '能力', value: v.abilities?.join(', ') || '' },
          { label: '事件', value: v.events?.join(', ') || '' },
          { label: '规则', value: v.rules?.join(', ') || '' },
          { label: '时间标记', value: v.timeMarkers?.join(', ') || '' },
  ...
  if (lastRetrievePlan) {
    ...
    builder.addNode(planId, '最近检索计划', 'rounded', 'planNode',
      createNodeDetail('检索计划', [
        ...
        { label: '已选标题', value: lastRetrievePlan.selectedTitles?.join(', ') || '' },
        { label: '检索模式', value: lastRetrievePlan.selectedModes?.join(', ') || '' },
      ]));
    ...
    for (let i = 0; i < recent.length; i++) {
      ...
      builder.addNode(nodeId, truncateText(label, 48), 'rounded', 'logNode',
        createNodeDetail(`日志: ${kindLabel}`, [
          ...
          { label: '丢弃原因', value: log.dropReasons?.join(', ') || '' },
          { label: '时间', value: timeStr },
        ]));
      builder.addEdge(groupId, nodeId);
    }
