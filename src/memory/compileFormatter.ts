...
function entitySearchText(e: NarrativeEntityCard): string {
  return [e.name, ...e.aliases, ...e.currentStatus, ...e.stableFacts, e.currentStance, ...e.affiliations].filter(Boolean).join(' ');
}
...
function formatEntityLine(e: NarrativeEntityCard): string {
  ...
  if (e.currentStatus.length > 0) parts.push(`｜状态：${e.currentStatus.join(', ')}`);
